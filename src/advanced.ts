...
import { darkTheme, lightTheme, bodyFont, type Theme } from './theme.js'
import type { DocumentLayout } from './layout.js'
import type { RenderOptions } from './index.js'
import { browserCanvasFactory, getDefaultFactory } from './canvas-provider.js'
...
export function shrinkwrap(markdown: string, options: ShrinkwrapOptions = {}): ShrinkwrapResult {
...
export function fitText(markdown: string, options: FitTextOptions): FitTextResult {
...
export function createOverlay(
  ...
  const highlights: Array<{ query: string; style: HighlightStyle }> = []
  ...
  function repaint(): void {
    ...
    for (const { query, style } of highlights) {
      ...
      for (const block of layout.blocks) {
        if (block.type !== 'paragraph' && block.type !== 'heading' && block.type !== 'blockquote') continue
...
function pointAtDistance(points: PathPoint[], d: number): { x: number; y: number; angle: number } | null {
  let accumulated = 0
  for (let i = 1; i < points.length; i++) {
...
export function animateText(
  ...
  function collectChars(): Array<{ x: number; y: number; char: string; font: string; color: string; index: number }> {
    const chars: Array<{ x: number; y: number; char: string; font: string; color: string; index: number }> = []
    let globalIndex = 0
...
export function updateRender(
  ...
  if (firstChanged === nextLines.length && prevMarkdown === nextMarkdown) return
