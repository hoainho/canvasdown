...
export type LaidOutBlock =
  | { type: 'heading';    level: 1|2|3|4|5|6; richLines: LaidOutRichLine[]; height: number; y: number }
  | { type: 'paragraph';  richLines: LaidOutRichLine[]; height: number; y: number }
  | { type: 'code_block'; lang: string; codeLines: string[]; lineHeight: number; height: number; y: number }
  | { type: 'blockquote'; richLines: LaidOutRichLine[]; height: number; y: number }
  | { type: 'list';       ordered: boolean; start: number; items: LaidOutListItem[]; height: number; y: number }
  | { type: 'table';      laidOut: LaidOutTable; height: number; y: number }
  | { type: 'image';      src: string; alt: string; width: number; height: number; y: number }
  | { type: 'hr';         height: number; y: number }
  | { type: 'space';      height: number; y: number }
...
function measureSpanWidth(text: string, font: string, lineHeight: number): number {
...
export function layoutRichSpans(
  spans: InlineSpan[],
  maxWidth: number,
  lineHeight: number,
): { richLines: LaidOutRichLine[]; height: number } {
  if (spans.length === 0) return { richLines: [], height: 0 }

  const fullText = spans.map((s) => s.text).join('')
  if (!fullText.trim()) return { richLines: [{ fragments: [], totalWidth: 0 }], height: lineHeight }

  const primaryFont = spans[0]!.font
  const prepared = prepareWithSegments(fullText, primaryFont)
  const { lines } = layoutWithLines(prepared, maxWidth, lineHeight)

  const richLines: LaidOutRichLine[] = []
  let charOffset = 0

  for (const line of lines) {
    const lineText = line.text
    const fragments: RenderedSpanFragment[] = []
    let fragX = 0
    let lineCharPos = 0
...
function layoutCodeBlock(
  ...
  maxWidth: number,
  theme: Theme,
): { codeLines: string[]; lineHeight: number; height: number } {
...
function layoutListItem(
  ...
  let childBlock: LaidOutListBlock | undefined
  let childHeight = 0
...
function layoutTable(
  ...
  const headerHeight = Math.max(
    ...headers.map((rl) => rl.length * theme.lineHeight),
    theme.lineHeight
  ) + theme.table.cellPadding * 2

  const rowHeights = rows.map((row) =>
    Math.max(...row.map((rl) => rl.length * theme.lineHeight), theme.lineHeight)
    + theme.table.cellPadding * 2
  )

  const colWidths = Array(colCount).fill(colWidth) as number[]
